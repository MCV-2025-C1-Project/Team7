...
def best_of_each_week():
    ...
    for name, img in qsd2_images.items():
        result = get_mask_and_crops(img_bgr=img, use_mask="mask_lab", min_area=1500)
        mask = result["mask"]
        crops = result["crops_img"]
        crops_masks = result["crops_mask"]
        ...
        # Añadir cada recorte como una nueva entrada
        for i, crop in enumerate(crops):
            new_qsd2_images[f"{name}_{i}"] = crop

    # Sustituimos las imágenes originales por los recortes
    qsd2_images = new_qsd2_images

    avg_precision = precision / len(qsd2_images)
    avg_recall = recall / len(qsd2_images)
    avg_F1 = F1 / len(qsd2_images)
    print(f"WEEK 2: Binary Mask metrics: Precision={avg_precision:.3f}, Recall={avg_recall:.3f}, F1={avg_F1:.3f}")
    ...
    for name, img in qst2_images.items():
        result = get_mask_and_crops(img_bgr=img, use_mask="mask_lab", min_area=1500)
        crops = result["crops_img"]
        cv2.imwrite(str(output_masks_dir / f"{name}.png"), result["mask"] * 255)

        for i, crop in enumerate(crops):
            new_qst2_images[f"{name}_{i}"] = crop

    qst2_images = new_qst2_images
