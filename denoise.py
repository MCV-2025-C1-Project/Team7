...
OUTIMG = Path("./denoise_out");   OUTIMG.mkdir(exist_ok=True, parents=True)
PLOTS  = Path("./denoise_plots"); PLOTS.mkdir(exist_ok=True, parents=True)
...
def nlm_colored(bgr, h=8, hColor=8, tw=7, sw=21):
    return cv2.fastNlMeansDenoisingColored(bgr, None, h=h, hColor=hColor,
                                           templateWindowSize=tw, searchWindowSize=sw)
...
for p in sorted(Q_DIR.glob("*")):
    if p.suffix.lower() not in SFX: continue
    ...
    rows.append(r); imgs.append((p, img, gray))
...
def pctl(arr, q): return float(np.percentile(arr, q)) if len(arr) else 0.0
...
p90_sp, p90_res, p90_sig, p90_hp = pctl(sp_arr,90), pctl(res_arr,90), pctl(sig_arr,90), pctl(hp_arr,90)
...
print(f"[INFO] Percentils p90 -> sp:{p90_sp:.4f}, res:{p90_res:.4f}, sig:{p90_sig:.2f}, hp:{p90_hp:.2f}")
print(f"[INFO] Llindars finals -> SP:{TH_SP:.4f} RES:{TH_RES:.4f} SIG:{TH_SIG:.2f} HP:{TH_HP:.2f}")

# Desa CSV de mètriques
with open(PLOTS / "noise_scores.csv", "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=["name","sp","res","sig","hp","sharp","path"])
...
def decide_and_denoise(bgr, metrics):
    ...
    sharp0 = lap_var(gray)
    reasons = []
    if metrics["sp"]  >= TH_SP:  reasons.append("SP")
    if metrics["res"] >= TH_RES: reasons.append("RES")
    if metrics["sig"] >= TH_SIG: reasons.append("SIG")
    if metrics["hp"]  >= TH_HP:  reasons.append("HP")
    ...
    if ("SP" in reasons) or ("RES" in reasons):
        out = cv2.medianBlur(out, 3); choice = "median 3x3"
        res_after = impulse_residue(cv2.cvtColor(out, cv2.COLOR_BGR2GRAY))
        if res_after > TH_RES:
            out = cv2.medianBlur(out, 5); choice = "median 5x5"
        # recuperar vores una mica
        out = bilateral(out, d=5, sC=20, sS=5); choice += " + bilateral"

    # Cas gaussian/alta-freq
    elif ("SIG" in reasons) or ("HP" in reasons):
        cand = []
        # NLM suau
        nlm = nlm_colored(bgr, h=8, hColor=8, tw=7, sw=21); cand.append(("NLM h8", nlm))
        # si molt nítida, prova alternatives i tria la millor per Laplacià post
        if sharp0 > 180.0:
            bil = bilateral(bgr, d=7, sC=25, sS=7);                cand.append(("bilateral d7 sC25 sS7", bil))
            gui = guided_like(bgr);                                cand.append(("guided-like", gui))
        ...
        for name, im in cand:
            ...
            diff = float(np.mean(np.abs(g.astype(np.int16) - gray.astype(np.int16))))/255.0
            score = sh - 50.0*diff
            if score > best_score:
...
def top_ids(arr, n): return np.argsort(-arr)[:n].tolist()
force_idx = set(
...
for i, (p, img, gray) in enumerate(imgs):
    metrics = {"sp": rows[i]["sp"], "res": rows[i]["res"], "sig": rows[i]["sig"], "hp": rows[i]["hp"]}
    ...
    if did_filter or (i in force_idx):
        ...
        ax[0].set_title(f"{p.name}\nsp={metrics['sp']:.4f} res={metrics['res']:.4f}\nσweak={metrics['sig']:.2f} hp={metrics['hp']:.2f}")
        ax[1].imshow(cv2.cvtColor(out, cv2.COLOR_BGR2RGB)); ax[1].axis("off")
        ax[1].set_title(f"Denoised: {choice}\nsharp {sharp0:.1f}→{sharp1:.1f}")
        plt.tight_layout()
        plt.savefig(PLOTS / f"{p.stem}__{('FORCED_' if not did_filter else '')}{choice.replace(' ','_')}.png", dpi=DPI)
        plt.close(fig)
